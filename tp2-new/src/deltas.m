function network = calculate_deltas(network, pattern)
	network.deltas{length(network.weights)}(pattern, :) = network.act_fn_der(network.outputs{end}(pattern), network.beta_fn) * (network.expected_outputs(pattern) - network.outputs{end}(pattern));
